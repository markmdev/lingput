...
describe("SessionService", () => {
  it("createSession proxies to repo", async () => {
    const repo: any = { createSession: jest.fn().mockResolvedValue({ sessionUUID: "u" }) };
  ...
  it("getSession proxies to repo", async () => {
    const repo: any = { getSession: jest.fn().mockResolvedValue({ sessionUUID: "u" }) };
    const svc = new SessionService(repo);
  ...
  it("updateSessionState proxies to repo", async () => {
    const repo: any = { updateSessionState: jest.fn().mockResolvedValue({ state: { a: 2 } }) };
    const svc = new SessionService(repo);
  ...
  it("completeSession proxies to repo", async () => {
    const repo: any = { completeSession: jest.fn().mockResolvedValue({ status: "completed" }) };
    const svc = new SessionService(repo);
